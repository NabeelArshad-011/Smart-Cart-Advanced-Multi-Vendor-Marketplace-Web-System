...
const findUserByEmail = (email) =>
  User.findOne({ email: new RegExp(`^${escapeEmailRegex(email)}$`, "i") });
...
router.post(
  ...
  catchAsyncErrors(async (req, res, next) => {
    try {
      ...
      const resetUrl = `http://localhost:3000/reset-password/user/${resetToken}`;

      await sendMail({
        email: user.email,
        subject: "Reset your password",
        message: `Hello ${user.name}, reset your password using this link: ${resetUrl}`,
      });
